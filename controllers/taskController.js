...
const dashboardStatistics = asyncHandler(async (req, res) => {
  console.log("📊 [dashboardStatistics] API hit.");

  try {
    // Check if req.user exists
    if (!req.user) {
      console.warn("❌ [dashboardStatistics] req.user is undefined.");
    ...
    if (isAdmin) {
      ...
      console.log("👤 Normal user - fetching team-based tasks");
    ...
    if (isAdmin) {
      ...
      console.log(`👥 Admin user - fetched ${users.length} users`);
    }
    ...
    console.log("📊 Task grouped by stage:", groupedTasks);

    // Graph data based on priority
    ...
    console.error("🔥 [dashboardStatistics] Server error:", error.stack || error.message);
